...
class SeleniumDemoReg
  ...
  def select_marital_option
    ...
    all = @chrome_driver.find_elements(:name, MARITAL_STATUS)
    all.each do |radio|
  ...
  def select_hobby_option
    ...
    all = @chrome_driver.find_elements(:name, HOBBY_STATUS)
    all.each do |radio|
      if radio['value'] == input
        radio.click
      end
  ...
  def get_selected_country
    @chrome_driver.find_element(:id, COUNTRY_DROP_DOWN_LIST).find_elements(:tag_name, 'option')
  end

  def country_dropdown_list_select(country)
    get_selected_country.each {|option| if option.text == country; option.click end}
  end
  ...
  def dob_month_list_select
    @chrome_driver.find_element(:id, DOB_MONTH_DROPDOWN_LIST).find_elements(:tag_name, 'option')[3].click
  end

  def dob_day_list_select
    @chrome_driver.find_element(:id, DOB_DAY_DROPDOWN_LIST).find_elements(:tag_name, 'option')[5].click
  end

  def dob_year_list_select
    @chrome_driver.find_element(:id, DOB_YEAR_DROPDOWN_LIST).find_elements(:tag_name, 'option')[6].click
  end
  ...
  # def valid_phone_number_checker
  #   @driver.find_element(:class, 'legend error').text
  # end
